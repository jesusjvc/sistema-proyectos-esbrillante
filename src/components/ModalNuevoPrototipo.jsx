    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={onCerrar}>
      <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-lg shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-700">
          <h3 className="font-semibold text-slate-800 dark:text-slate-100">Nuevo prototipo</h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          {proyectos && (
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Proyecto</label>
              ...
              {[['prototipo', 'Prototipo'], ['pagina_web', 'Página web']].map(([v, l]) => (
                <button
                  ...
                  className={`flex-1 text-sm font-medium py-2 rounded-lg border transition-colors ${
                    tipo === v ? 'border-brand-500 bg-brand-50 dark:bg-brand-500/10 text-brand-800 dark:text-brand-300' : 'border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-600'
                  }`}
                >
                  {l}
                </button>
          ...
          {tipo === 'pagina_web' && (
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">¿Cómo la vas a compartir?</label>
              <div className="flex gap-2">
                {[['html', 'Subir HTML'], ['url', 'URL externa (Elementor, Vercel...)']].map(([v, l]) => (
                  <button
                    ...
                    className={`flex-1 text-xs font-medium py-2 rounded-lg border transition-colors ${
                      modo === v ? 'border-brand-500 bg-brand-50 dark:bg-brand-500/10 text-brand-800 dark:text-brand-300' : 'border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-600'
                    }`}
                  >
                    {l}
                  </button>
          ...
          {modo === 'html' ? (
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Archivo HTML</label>
              <label className="flex flex-col items-center justify-center gap-1 border-2 border-dashed border-slate-200 dark:border-slate-700 hover:border-brand-300 dark:hover:border-brand-600 rounded-xl py-6 cursor-pointer transition-colors">
                <input type="file" accept=".html,text/html" className="hidden" onChange={(e) => cargarArchivo(e.target.files[0])} />
                <span className="text-sm text-slate-500 dark:text-slate-400">{nombreArchivo || 'Haz clic para seleccionar un archivo'}</span>
              </label>
            </div>
          ) : (
            <div>
