    ...
    <div className="border-t border-slate-100 dark:border-slate-700 pt-3 mt-3">
      ...
      {comentarios.length > 0 && (
        <div className="space-y-2.5 mb-3 max-h-64 overflow-y-auto">
          {comentarios.map((c) => (
            <div key={c.id} className="text-sm bg-slate-50 dark:bg-slate-900 rounded-lg px-3 py-2">
              <div className="flex items-baseline gap-2">
                <span className="font-medium text-slate-700 dark:text-slate-300">{c.autor}</span>
                <span className="text-[11px] text-slate-400 dark:text-slate-500">{formatFechaHora(c.creadoEn)}</span>
              </div>
              <div className="text-slate-600 dark:text-slate-300 whitespace-pre-wrap mt-0.5">{resaltarMenciones(c.texto, nombresConocidos)}</div>
            </div>
          ))}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="relative">
        <textarea
          ...
          rows={2}
          className="w-full text-sm border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-brand-400 dark:focus:ring-brand-500/40 focus:border-transparent resize-none"
        />

        {picker && opcionesPicker.length > 0 && (
          <div className="absolute z-10 bottom-full mb-1 left-0 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg py-1 w-56 max-h-48 overflow-y-auto">
            {opcionesPicker.map(([id, nombre]) => (
              <button
                key={id}
                type="button"
                onClick={() => elegirMencion(id, nombre)}
                className="w-full text-left px-3 py-1.5 text-sm text-slate-700 dark:text-slate-300 hover:bg-brand-50 dark:hover:bg-brand-500/15 hover:text-brand-800 dark:hover:text-brand-300"
              >
                {nombre}
              </button>
            ))}
