// Datos puros del catálogo builtin de plantillas (sin dependencias de Vite ni
// del navegador) — usados por src/data/plantillas.js (re-exportados) y por
// server/src/scripts/migrarPlantillasABD.js, que los importa directo para no
// depender de src/data/api.js (que usa import.meta.env, inexistente en Node).
...
// Exportada para server/src/scripts/migrarPlantillasABD.js (script one-off que
// importa esta función pura para migrar el catálogo builtin a la base de datos).
...
// Exportada por el mismo motivo que seedPlantillas: server/src/scripts/migrarPlantillasABD.js
// la usa directamente (con sus ids locales originales intactos, antes de que
// seedPlantillas() los regenere con uid() al armar cada plantilla builtin) para
// poder remapear las dependencias correctamente al migrar a la base de datos.
export function tareasWebBase() {
  return [
    // ── Fase 1: Arranque ──────────────────────────────────────────────────
